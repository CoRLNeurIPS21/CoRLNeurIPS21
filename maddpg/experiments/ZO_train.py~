...
def update_critic_lstm(trainers, obs_n, action_n, p_states):
    ...
    for trainer in trainers:
        q_val, (trainer.q_c, trainer.q_h) = trainer.q_debug['q_values'](
            *(obs_n + action_n + q_c_n + q_h_n))
    ...
    with U.single_threaded_session() as sess:
        ...
        while True:
            ...
            if arglist.critic_lstm:
                q_in_c_n, q_in_h_n = get_lstm_states(
                    'q', trainers)  # num_trainers x 1 x 1 x 64
            ...
            if arglist.critic_lstm:
                ...
                q_out_c_n, q_out_h_n = get_lstm_states(
                    'q', trainers)  # num_trainers x 1 x 1 x 64
            if arglist.actor_lstm:
            ...
            if arglist.tracking:
                for i, a in enumerate(trainers):
                    if arglist.num_episodes - len(episode_rewards) <= 1000:
                        a.tracker.record_information("goal", np.array(
                            env.world.landmarks[0].state.p_pos))
                        a.tracker.record_information(
                            "position", np.array(env.world.agents[i].state.p_pos))
                    a.tracker.record_information("ag_reward", rew_n[i])
                    a.tracker.record_information(
                        "team_dist_reward", info_n["team_dist"][i])
                    a.tracker.record_information(
                        "team_diff_reward", info_n["team_diff"][i])
            ...
            if arglist.sync_sampling:
                inds = [random.randint(0, len(
                    trainers[0].replay_buffer._storage)-1) for i in range(arglist.batch_size)]
            else:
                inds = None
